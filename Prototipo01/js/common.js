...
async function renderFooterData() {
  ...
  animarContagem(spanTotalCharacters, totalCharacters);
  animarContagem(spanTotalLocations, totalLocations);
  animarContagem(spanTotalEpisodes, totalEpisodes);

  const spanDevName = document.getElementById("dev-name");
  spanDevName.innerText = "Elaine Cristina";

  const spanCurrentYear = document.getElementById("current-year");
  spanCurrentYear.innerText = new Date().getFullYear();
}

function animarContagem(elemento, valorFinal, duracao = 2000) {
  let inicio = 0;
  let incremento = valorFinal / (duracao / 30);
